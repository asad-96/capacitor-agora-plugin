...
//    }
    ...
    open func rtcEngine(_ engine: AgoraRtcEngineKit, networkQuality uid: UInt, txQuality: AgoraNetworkQuality, rxQuality: AgoraNetworkQuality) {
        ...
        updateStats(for: uid, with: txQuality)
    }
...
extension AgoraVideoViewer {
    func updateStats(for uid: UInt, with stats: AgoraNetworkQuality) {
        ...
        if let videoFeed = self.videoLookup[uid] {
            ...
            if uid == self.userID {
                delegate?.didChangeVideoConfig(event: "network-quality", data: [:])
            }
        }
      }
}
