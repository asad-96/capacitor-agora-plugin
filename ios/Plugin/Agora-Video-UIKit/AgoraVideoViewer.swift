...
public protocol AgoraVideoViewerDelegate: AnyObject {
    ...
    func didChangeVideoConfig(event: String, data: Any)
...
open class AgoraVideoViewer: MPView, SingleVideoViewDelegate {
    ...
    lazy internal var streamerCollectionView: MPCollectionView = {
        ...
        //        let smallerDim = AgoraCollectionViewer.pinnedCellSize
        ...
        case .top, .bottom:
            ...
            //            collView.frame.size = CGSize(width: self.bounds.width, height: collectionViewH)
            //            if floatPos == .top {
            //                #if os(iOS)
            //                collView.frame.origin =  CGPoint(x: 0, y: UIScreen.main.bounds.height - bottomViewH - collectionViewH)
            ...
            //            } else {
            //                #if os(iOS)
            //                collView.frame.origin = CGPoint(x: 0, y: self.bounds.height - collectionViewH)
            ...
            let controlContainer = getControlContainer()
            NSLayoutConstraint.activate([
                collView.leftAnchor.constraint(equalTo: self.leftAnchor),
                collView.rightAnchor.constraint(equalTo: self.rightAnchor),
                collView.bottomAnchor.constraint(equalTo: controlContainer.topAnchor, constant: bottomOffset),
                collView.heightAnchor.constraint(equalToConstant: collectionViewH)
            ])
            
        case .right, .left:
            (collView.collectionViewLayout as? MPCollectionViewFlowLayout)?.scrollDirection = .vertical
            collView.frame.size = CGSize(width: collectionViewH, height: self.bounds.height)
            if floatPos == .left {
                collView.frame.origin = .zero
#if os(iOS)
                ...
                collView.frame.origin = CGPoint(x: self.bounds.width - collectionViewH, y: 0)
#if os(iOS)
